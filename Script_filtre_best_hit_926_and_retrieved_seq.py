...
# output_file_path = '/Users/trystan.bessonnier/Desktop/Blast/best_hits_926.txt'
...
# Chemin du fichier FASTA nettoyé
cleaned_fasta_path = '/Users/trystan.bessonnier/Desktop/Blast/cleaned_retrieved_sequence_seqids_Protein_overexpressed_shrink_LFC_gene_926_ids.fasta'
# Chemin du fichier FASTA sans duplicats
unique_fasta_path = '/Users/trystan.bessonnier/Desktop/Blast/unique_retrieved_sequence_seqids_Protein_overexpressed_shrink_LFC_gene_926_ids.fasta'

# Dictionnaire pour stocker les séquences uniques
unique_sequences = {}

# Lire le fichier FASTA nettoyé et identifier les séquences uniques
with open(cleaned_fasta_path, 'r') as fasta_file:
    for record in SeqIO.parse(fasta_file, 'fasta'):
        if record.id not in unique_sequences:
            unique_sequences[record.id] = record

# Écrire les séquences uniques dans un nouveau fichier FASTA
with open(unique_fasta_path, 'w') as output_file:
    SeqIO.write(unique_sequences.values(), output_file, 'fasta')

print(f'Le fichier FASTA sans duplicats a été créé : {unique_fasta_path}')
...
# Lire le fichier FASTA sans duplicats et extraire les séquences correspondant aux RefSeq IDs
sequences_to_write = []
with open(unique_fasta_path, 'r') as fasta_file:
    for record in SeqIO.parse(fasta_file, 'fasta'):
        if record.id.strip() in refseq_ids:
            sequences_to_write.append(record)
...
# Chemin du fichier FASTA sans duplicats
unique_fasta_path = '/Users/trystan.bessonnier/Desktop/Blast/unique_retrieved_sequence_seqids_Protein_overexpressed_shrink_LFC_gene_926_ids.fasta'
# Chemin du fichier de sortie pour les en-têtes
headers_output_path = '/Users/trystan.bessonnier/Desktop/Blast/protein_headers.txt'

# Lire le fichier FASTA et extraire les en-têtes
headers = []
with open(unique_fasta_path, 'r') as fasta_file:
    for record in SeqIO.parse(fasta_file, 'fasta'):
        headers.append(record.description)  # Utiliser .description pour obtenir l'en-tête complet

# Écrire les en-têtes dans un nouveau fichier
with open(headers_output_path, 'w') as output_file:
    for header in headers:
        output_file.write(f'>{header}\n')

print(f'Le fichier des en-têtes de protéines a été créé : {headers_output_path}')



from Bio import SeqIO

# Chemin du fichier FASTA des meilleures séquences
fasta_path = '/Users/trystan.bessonnier/Desktop/Blast/best_hits_sequences_926.fasta'
# Chemin du fichier de sortie pour les en-têtes
headers_output_path = '/Users/trystan.bessonnier/Desktop/Blast/protein_headers_from_best_hits.txt'
